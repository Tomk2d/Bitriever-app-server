...
public class CoinPriceTickerController {
     ...
     */
    @Operation(summary = "전체 코인 현재가 조회", description = "서버에서 관리하는 모든 코인의 현재 가격을 조회합니다. 클라이언트 최초 연결 시 사용합니다.")
    @ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공"),
    })
    @GetMapping("/all")
    public ApiResponse<List<CoinTickerPriceDto>> getAllCoinPrices() {
        List<CoinTickerPriceDto> prices = coinPriceTickerService.getAllCoinPrices();
